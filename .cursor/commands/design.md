...
## ツールガイダンス
- **最初に読み込み**: アクション前にすべてのコンテキストを読み込み（specs、steering、templates、rules）
...
- **提案アクション**: "`/spec-design $1 -y` を実行して要件を自動承認して進行"

**要件が見つからない**:
- **実行停止**: 要件ドキュメントが存在しなければならない
- **ユーザーメッセージ**: "`.cursor/$1/requirements.md` に requirements.md が見つかりません"
- **提案アクション**: "先に `/spec-requirements $1` を実行して要件を生成してください"
...
**ステアリングコンテキストが見つからない**:
- **警告**: "ステアリングディレクトリが空または見つかりません - 設計がプロジェクト標準と整合しない可能性があります"
...
- その後 `/spec-tasks $1 -y` で実装タスクを生成

**修正が必要な場合**:
- フィードバックを提供し `/spec-design $1` を再実行
