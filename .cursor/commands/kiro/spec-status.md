...
- `.cursor/specs/$1/spec.json` からメタデータとフェーズステータスを読み込み
- 既存ファイルを読み込み: `requirements.md`、`design.md`、`tasks.md`（存在する場合）
- `.cursor/specs/$1/` ディレクトリで利用可能なファイルを確認
...
**Specが見つからない**:
- **メッセージ**: "`$1` のspecが見つかりません。`.cursor/specs/` で利用可能なspecを確認してください"
...
- `.cursor/specs/` 内のすべてのspecをステータスとともに表示
