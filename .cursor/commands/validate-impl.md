...
**引数が提供されない場合**（`$1` が空）:
- 会話履歴から `/spec-impl <feature> [tasks]` コマンドを解析
- 各実行から機能名とタスク番号を抽出
...
### エラーシナリオ
- **実装が見つからない**: 履歴に `/spec-impl` がなく `[x]` タスクもない場合、"実装が検出されませんでした" と報告
...
**NO-GO判定の場合**:
- リストされた重大な問題に対処
- `/spec-impl <feature> [tasks]` を再実行して修正
- `/validate-impl [feature] [tasks]` で再検証
