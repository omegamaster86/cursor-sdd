...
### エラーシナリオ
- **設計が見つからない**: design.md が存在しない場合、メッセージで停止: "先に `/spec-design $1` を実行して設計ドキュメントを生成してください"
...
- `/spec-tasks $1` を実行して実装タスクを生成
- または `/spec-tasks $1 -y` で自動承認して直接進行

**設計の修正が必要な場合（NO-GO判定）**:
- 特定された重大な問題に対処
- `/spec-design $1` を改善とともに再実行
- `/validate-design $1` で再検証
