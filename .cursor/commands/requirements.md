...
- **成功基準**:
  - ステアリングコンテキストと整合した完全な要件ドキュメントを作成
...
## ツールガイダンス
- **最初に読み込み**: 生成前にすべてのコンテキスト（spec、steering、rules、templates）を読み込み
...
- **ステアリングディレクトリが空**: プロジェクトコンテキストが不足しており、要件品質に影響する可能性があることを警告
...
- その後 `/spec-design $1 -y` で設計フェーズに進む

**修正が必要な場合**:
- フィードバックを提供し `/spec-requirements $1` を再実行
